...
const NewsDetail = () => {
  ...
  useEffect(() => {
    const fetchElement = async () => {
      try {
        ...
        const imgUrl = articleDocument
          .querySelector("picture source[data-hero]")
          .getAttribute("srcset");
        const imgAlt = articleDocument
          .querySelector("picture img[data-hero]")
          .getAttribute("alt");
  ...
  function saveFavorite() {
    if (isFavorite) {
      localStorage.setItem(
        "favorites",
        JSON.parse(favorites).filter((f) => f !== url)
      );
      setIsFavorite(false);
      return;
    }
  ...
  function renderParagraphs(paragraphs) {
    return paragraphs.map((paragraph, ix) => <p key={ix}>{paragraph}</p>);
  }
